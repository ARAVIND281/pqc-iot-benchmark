...
As illustrated in Table \ref{tab:lit_review}, our framework uniquely bridges these silos by evaluating the exact August 2024 FIPS standards concurrently against all three RFC 7228 memory boundaries ($<10$KB, $<50$KB, and $<250$KB), mapping strictly normalized execution times into a comprehensive 10-point TOPSIS decision matrix. Furthermore, while prior literature rarely combines dynamic CPU energy cycles with deterministic RF packet transmission wattage, our hybrid evaluation ($E_{total} = E_{cpu} + E_{tx}$) acts as the first unified embedded energy baseline.
