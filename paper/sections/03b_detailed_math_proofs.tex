...
To justify the aggressive processing bottlenecks measured in Section \ref{sec:results}, we must explicitly formalize the mathematical structures executed within the microcontroller ALUs.
...
The Decisional MLWE problem challenges an adversary to distinguish between samples drawn natively from $\mathcal{A}_{\mathbf{s},\chi}$ versus samples drawn completely uniformly from $R_q^{k \times k} \times R_q^k$.
...
\end{equation}
where $\circ$ is pointwise multiplication. The forward NTT evaluates a polynomial $a(X)$ at the $n$ primitive roots of unity modulo $q$. The Cooley-Tukey butterfly algorithm natively reduces this to $\mathcal{O}(n \log n)$. However, the Cortex-M0 (Class 0 architecture) fundamentally lacks the 32-bit fast multiply-accumulate (\texttt{MAC}) instructions required to rapidly execute the Barrett or Montgomery reductions necessary after every butterfly addition, explaining the profound latency divergence between C1 (Cortex-M0) and C2 (Cortex-M4) endpoints graphed dynamically in Figure \ref{fig:keygen_time}.

\subsection{Formalization of Hash-Based Trees (SLH-DSA)}

Unlike LWE, SLH-DSA secures natively upon generic collision-resistant cryptographic hashing boundaries. The algorithmic structure constructs a stateful signature tree defined by height $h = h' \cdot d$, containing $d$ interconnected levels.
