...
Simulated benchmarks invite an obvious objection: Python timing on a desktop CPU is not the same as cycle-counting on a Cortex-M4. We address this head-on. Our baseline instruction counts derive from the hand-optimized \texttt{pqm4} assembly routines \cite{kannwischer2019pqm4}, which have been validated against physical hardware by the cryptographic community. We then impose RFC 7228 memory ceilings using Python's \texttt{resource.setrlimit}, triggering genuine OOM crashes when an algorithm exceeds the C0/C1/C2 bounds — not synthetic error codes, but actual segfaults.
...
Hardware profiling requires strict isolation. For every algorithmic configuration tested against an RFC 7228 device tier, we deployed a structured test harness. Our protocol doesn't just measure wall-clock execution time; it actively forces the host kernel to enforce RSS heap allocation ceilings during operation. 
...
Raw data is inherently dirty. OS-level interrupts inevitably jitter execution timings. Before piping results into our ANOVA matrices, we scrubbed the traces using a rigorous, unsupervised Interquartile Range (IQR) filter to aggressively purge these environmental stochastic anomalies.
