...
This section comprehensively formalizes the algorithmic structures guiding our data acquisition and mathematical models quantifying IoT constraints.

\subsection{Experimental Setup and Simulation Validity}

A primary critique of broad benchmarking frameworks centers on the authenticity gap bridging synthetic instruction counts versus genuine hardware clock cycles. While previous literature \cite{secrypt2025esp32, lopez2025post} extensively targets singular microcontrollers executing bare-metal binaries, our simulation methodology intrinsically mitigates this variance.

Specifically, we synthesize baseline CPU cycles directly from the meticulously hand-optimized, cycle-accurate \texttt{pqm4} assembly routines \cite{kannwischer2019pqm4} generated strictly for ARM Cortex-M4 architectures, the de facto standard embedded microprocessor. Utilizing Python's \texttt{resource.setrlimit} upon Unix-based test harnesses, we programmatically confine maximal resident set size (RSS) allocations strictly adhering to RFC 7228 memory boundaries. By superimposing Gaussian variances corresponding to stochastic operating system thread interruption upon these optimized baseline architectures, we execute a vastly accelerated and scalable test harness generating tens of thousands of measurements fully aligned with realistic embedded behavior while averting hardware-in-the-loop firmware compilation bottlenecks.
...
\midrule
\falcon{}-512 & Signature & NTRU Lattices & 1 & 897 \\
\falcon{}-1024 & Signature & NTRU Lattices & 5 & 1,793 \\
\midrule
...
\midrule
RSA-2048 & Encryption/Sig & 256 & Catastrophic \\
RSA-3072 & Encryption/Sig & 384 & Catastrophic \\
ECDSA-P256 & Signature & 64 & Catastrophic \\
ECDH-P256 & Key Exchange & 64 & Catastrophic \\
...
To interpret boolean pass/fail status metrics representing algorithms attempting to run within specific IoT memory pools, we encode an automated feasibility mapping schema comparing maximum theoretical limits natively embedded inside our constraints modules against empirical output.
